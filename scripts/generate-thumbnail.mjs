...
async function generateThumbnailFromMarkdown(mdxPath) {
  try {
    ...
    const webpBuffer = await sharp(pngData).webp({ quality: 100 }).toBuffer();

    const relativePath = mdxPath.replace(/\/[^\/]+\.mdx?$/, "");
    const thumbnailPath = path.join(relativePath, "thumbnail.webp");
    writeFileSync(thumbnailPath, webpBuffer);
